    ...
    return SafeArea(
      child: BlocConsumer<PagesBloc, PagesState>(
        ...
        builder: (context, state) {
          if (state.categoryModel!.data!.length > 0 && j == 1) {
            for (var ba in state.categoryModel!.data!) {
              activities.add(Banks(ba.id!, ba.name!, 0));
              j++;
            }
          ...
          return BlocConsumer<SubscriptionsBloc, SubscriptionsState>(
            bloc: bloc,
            listener: (context, state) {
              if (state.packagesModel!.data!.length > 0 && i == 1) {
                for (var ba in state.packagesModel!.data!) {
                  _plans.add(Banks(ba.id!, ba.name!, ba.days!));
                  i++;
                }
              ...
              if (state.isSuccessSub!)
                Navigator.pushReplacement(context,
                    MaterialPageRoute(builder: (context) => SuccScreen()));
            },
            builder: (context, state) {
              return Scaffold(
                body: ModalProgressHUD(
                  inAsyncCall: state.isLoadingSub!,
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    ElevatedButton(
                                      ...
                                      child: Text("تجديد اشتراكك"),
                                    ),
                                  ],
                                ),
                              )
                            : BlocConsumer<SubscriptionsBloc,
                                SubscriptionsState>(
                                bloc: bloc,
                                listener: (context, state) {},
                                builder: (context, state) {
